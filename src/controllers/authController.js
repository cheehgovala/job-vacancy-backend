  ...
  try {
    ...
    if (userExists) {
      if (userExists.isVerified) {
        ...
        // Send OTP — awaited so failures are logged clearly
        const emailSent = await sendOTPEmail(email, otp);
        if (!emailSent) console.error('[Register] OTP email failed for', email);
    ...
    const user = await User.create({
      ...
      employerProfile: role === 'employer' ? { companyName: name } : undefined,
      isVerified: false,
      otp: otp,
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    // Send reset OTP email — awaited so failures are logged clearly
    const emailSent = await sendOTPEmail(email, otp);
    if (!emailSent) console.error('[ForgotPassword] OTP email failed for', email);
...
export const resetPassword = async (req, res) => {
  try {
    ...
    const isMatch = (otp === user.resetPasswordOtp);
    if (!isMatch) {
      return res.status(400).json({ error: 'Invalid OTP' });
    }
